...
export default class AutoFlatList extends PureComponent {
  ...
  loadData = () => {
    ...
    fetchData(this.page)
      .then(data => this.setData(data.data.records))
      .catch(e => {
        this.setState({
          isLoading: false,
        });
    ...
    if (error) {
      return (
        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
          <Text onPress={this.props.reset}>网络出错了点击重试</Text>
        </View>
      );
    }
      ...
      if (this.props.ListHeaderComponent) {
        ...
        return (
          <View style={{ justifyContent: 'center', alignItems: 'center', flex: 1 }}>
            <Image source={Images.common.nodata} style={{ marginTop: -100 }} />
    ...
    return enableLoadMore ? (
      <View style={styles.loadMoreView}>
        ...
        <Text
          ...
          // }}
        >
          {isEndRefresh ? '正在加载中...' : isNoMoreData ? '------到底了------' : ''}
        </Text>
      </View>
    ) : null;
  }
